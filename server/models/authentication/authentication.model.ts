...
export async function forgotPassword(username: string) {
  try {
    ...
    if (user && user.type==="thirdParty")
      return {
        error:
          "The password reset for provided email address is handled by external third party email provider (applicable for when login it with a Google account).",
      };
...
export async function resetPassword(query: ParsedQs) {
  try {
    ...
    const passwordResetToken = await findPasswordResetToken(
      query.username as ParsedQs
    );
    const now = new Date();
    const maxAge = 1 * 60 * 60 * 1000;
    if (
      passwordResetToken === "" ||
      now.getTime() - passwordResetToken.tokenCreationTimestamp.getTime() >
        maxAge
    ) {
