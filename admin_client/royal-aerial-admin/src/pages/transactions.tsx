...
const Transactions: React.FC = () => {
  ...
  const onView = (id: string) => {
    ...
    if (existingTransaction) {
      const filteredTransactionDetails = {
        ...
        participantDetails: `First Name: ${
          existingTransaction.participantDetails.firstName
        }, 
        Last Name: ${existingTransaction.participantDetails.lastName},
        Birth Date: ${formatDateToBelgium(
          existingTransaction.participantDetails.birthDate
        )},
        ...
        courseDetails: existingTransaction.courseDetails
          .map(
            (course, index) =>
              `Course ${index + 1} - Course ID: ${course.courseId}, Quantity: ${
                course.quantity
              }`
          )
          .join(", "),
  ...
  const onDelete = (id: string) => {
    const findTransaction = transactions?.items.find(
      (transaction) => transaction._id === id
    );
    if (findTransaction) {
  ...
  //Row actions
  const rowActions = [{ view: onView, delete: onDelete }];
